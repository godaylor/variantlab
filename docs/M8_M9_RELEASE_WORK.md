...
The repository and browser-local demo are publishable. General production Connected beta still depends on external managed Postgres/Redis/object storage, secrets, HTTPS/observability/retention configuration, trademark clearance, legal clearance for the dependency/codec distribution chain, and independent assistive-technology/user acceptance. Technical inventories and source offers are complete evidence inputs, not legal advice.
