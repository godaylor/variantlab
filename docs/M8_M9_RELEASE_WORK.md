...
Additional CI findings fixed during publication: sequence-level commits preserve an in-progress scene-name draft; media imports wait for persisted-job initialization; read-only M7 baseline inspection does not create an empty IndexedDB schema. Preview object URLs now depend on the selected artifact path rather than every job-progress emission. Rust plans 250 ms proxy keyframe intervals to bound seek decode work; M3 measures the completed seek-optimized proxy. Regression browser coverage exercises delayed IndexedDB initialization and stable preview URLs across worker failure. Cargo fetch makes public crate sources readable for the host-side license packager.
