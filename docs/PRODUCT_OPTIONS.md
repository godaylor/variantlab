...
Этот документ сравнивает ровно три самостоятельных продукта, которые можно построить на editor/media-ядре OpenCut Classic. Речь не о ребрендинге универсального видеоредактора: каждый вариант меняет основную сущность продукта, рабочий процесс, модель состояния и коммерческое обещание.
...
- сохранить MIT `LICENSE`, сведения об исходной open-source основе и допустимую атрибуцию OpenCut;
- не использовать название и логотип OpenCut как марку нового продукта;
...
### Чем это отличается от OpenCut

OpenCut Classic организован вокруг ручного монтажа отдельной scene/timeline. VariantLab организован вокруг **master-to-many dependency graph**. Пользователь редактирует правило и семейство, а не вручную дублирует проект двенадцать раз.
...
Это станет самостоятельным продуктом только при наличии настоящей event/timecode модели. Набор неоновых overlays поверх обычного редактора останется skin OpenCut и не оправдает трансформацию.
...
`VariantLab` — рабочее кодовое имя для спецификации и портфолио. До публичного запуска необходимы отдельная проверка товарного знака, домена и конфликтующих продуктов. Новый продукт не должен называться OpenCut, использовать его логотип или создавать впечатление официального продолжения. Допустимая формулировка происхождения и полный набор сторонних notices должны быть зафиксированы до релиза; корневой `LICENSE` и сведения об исходной open-source основе сохраняются.
