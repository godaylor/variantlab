...
for (const entry of readdirSync(bunStore)) {
  const modules = path.join(bunStore, entry, "node_modules");
  if (!existsSync(modules)) continue;
  const packageDirs = readdirSync(modules, { withFileTypes: true }).filter(v => v.isDirectory()).flatMap(v => v.name.startsWith("@") ? readdirSync(path.join(modules,v.name)).map(n => path.join(modules,v.name,n)) : [path.join(modules,v.name)]);
