...
function statusLabel(saveState: SaveState, locale: VariantLabLocale): string {
	...
	if (saveState === "error") return locale === "ru" ? "Ошибка сохранения" : "Save failed";
...
export function VariantLabStudio() {
	...
	const visibleNotice = notice || t({ ru: "Создайте кампанию или импортируйте проект исходного редактора.", en: "Create a campaign or import a source-project." });
	...
	return (
		<main className="min-h-screen bg-[#e7e9e6] text-[#172128]">
				...
				<div className="mx-auto flex max-w-[1500px] items-end justify-between gap-6">
					<div>
						<p className="font-mono text-[11px] tracking-[0.22em] text-[#48606d] uppercase">
							{t({ ru: "Креативные операции / локальная надёжность", en: "Creative operations / local durability" })}
						</p>
					...
					>
						...
						<p>{t({ ru: "ревизия", en: "revision" })} {state?.campaign.revision ?? "—"}</p>
					</div>
				</div>
			</header>
			...
			<div className="mx-auto grid max-w-[1500px] grid-cols-1 lg:grid-cols-[270px_minmax(0,1fr)]">
				<aside className="border-b-2 border-[#172128] bg-[#d9ddd9] p-5 lg:min-h-[calc(100vh-91px)] lg:border-r-2 lg:border-b-0">
					...
					<h2 className="mt-8 border-b border-[#172128] pb-2 font-mono text-xs font-bold uppercase">
						{t({ ru: "Импорт исходных проектов", en: "Source-project imports" })}
					</h2>
					{legacyProjects.length === 0 ? (
						<p className="mt-3 text-xs leading-5 text-[#48606d]">
							{t({ ru: "Старые проекты не найдены. Исходное пространство данных не изменяется.", en: "No legacy projects found. The source namespace remains untouched." })}
						</p>
					) : (
				...
				<section className="min-w-0 p-5 lg:p-8">
					{!state ? (
						<div className="border-2 border-dashed border-[#48606d] p-10">
							<h2 className="text-xl font-black">{t({ ru: "Кампания не открыта", en: "No campaign open" })}</h2>
							<p className="mt-2 max-w-xl text-sm text-[#48606d]">{visibleNotice}</p>
						</div>
					) : (
						<>
							<div className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-[#172128] pb-5">
								<div>
									<p className="font-mono text-[11px] uppercase">
										{t({ ru: "Кампания / мастер-последовательность", en: "Campaign / master sequence" })}
									</p>
							...
							<div className="mt-6 lg:hidden">
								<div className="border-2 border-[#172128] bg-[#f6f7f4] p-5">
									...
									<p className="mt-2 text-sm">
										{t({ ru: "Полное редактирование таймлинии доступно от 1024 px. Здесь остаются состояние кампании и статус сохранения.", en: "Full timeline editing starts at 1024 px. Campaign state and save health remain available here." })}
									</p>
							...
							<div className="mt-6 hidden grid-cols-[220px_minmax(0,1fr)] gap-6 lg:grid">
								...
								<div className="border-2 border-[#172128] bg-[#f6f7f4]">
									<div className="grid grid-cols-[1fr_auto] border-b-2 border-[#172128] bg-[#b9d1dc]">
										<div className="p-5">
											<p className="font-mono text-[11px] uppercase">
												{t({ ru: "Активная сцена", en: "Active scene" })} / {activeScene?.id}
											</p>
											<h3 className="mt-2 text-2xl font-black">
												{activeScene?.name}
											</h3>
										</div>
										<div className="flex items-center border-l-2 border-[#172128] px-5 font-mono text-xs">
											MASTER
										</div>
									</div>
									...
									<div className="p-5">
										...
										<div className="mt-6 flex flex-wrap gap-2 border-t border-[#a9b1ad] pt-5">
											...
											>
												{t({ ru: "Отменить в активной сцене", en: "Undo active scene" })}
											</button>
											<button
												type="button"
												...
												className="border-2 border-[#172128] px-4 py-2 text-sm font-bold focus-visible:outline-4 focus-visible:outline-[#d26532]"
											>
												{t({ ru: "Повторить в активной сцене", en: "Redo active scene" })}
											</button>
											<button
												type="button"
												onClick={() => {
											...
											>
												{t({ ru: "Сломать следующую запись", en: "Fail next write" })}
											</button>
										</div>
										...
										{saveState === "error" && (
											...
											>
												<p className="font-bold">
													{t({ ru: "Сохранение не удалось — кампания изменена", en: "Save failed — campaign is dirty" })}
												</p>
							...
							<ConnectedCloudBoard state={state} onNotice={setNotice} />
							<ConnectedSyncBoard key={`sync:${state.campaign.id}`} state={state} isDirty={isDirty} onOpen={async (id) => { await refreshLists(); await openCampaign(id); }} />
							...
							<div className="mt-6 grid grid-cols-2 border-2 border-[#172128] bg-[#172128] font-mono text-xs text-white lg:grid-cols-4">
								...
								<div className="border-r border-[#66808d] p-3">
									<span className="block text-[#aac0ca]">WRITE P95 TARGET</span>
									&lt; 100 MS
								</div>
								<div className="p-3">
			...
			<footer className="border-t-2 border-[#172128] bg-[#f6f7f4] px-5 py-3 text-xs text-[#48606d] lg:px-8">
				{t({ ru: "VariantLab — рабочее название. Сведения об исходном открытом проекте, лицензия MIT и авторские уведомления сохранены в разделе атрибуции.", en: "VariantLab is a working codename. Source-project attribution, the MIT license and copyright notices are preserved in the attribution section." })}
			</footer>
		</main>
	);
}
